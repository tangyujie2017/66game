 ...
 */
var currentValidFormId;
(function(c) {
	var remoteCounts = 0;
	c.extend(c.fn,{
		validate : function(a) {
			if (this.length) {
				//currentValidFormId = this.selector;
	...
	c.extend(c.validator,{
				...
				prototype : {
					...
					checkForm : function() {
							...
							elements : function() {
								var a = this, b = {};
								currentValidFormId = this.currentForm;
...
//									return e.valid;
								...
								g[b.name] = a;
								remoteCounts++;
								var result = c.ajax(c.extend(true,{
									url : d,			
									mode : "abort",	
			...
			&& document.addEventListener && c.each({
				...
				function d(e) {
					...
					if(isnull(c.event.handle)){
						if(b=="focusout"){
							$(currentValidFormId).find("input").unbind("change");
							$(currentValidFormId).find("input").change(function(){
								if(!isnull($(this).val())){
									$(currentValidFormId).valid();
								}
							});
						}
						return null;
