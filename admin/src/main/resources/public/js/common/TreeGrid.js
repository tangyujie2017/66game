	...
	init = function(){
		...
		if((_config.hoverRowBackground || "false") == "true"){
			__root.find("tr").hover(
				function(){
					if(jQuery(this).attr("class") && jQuery(this).prop("class") == "header") return;
		...
		__root.find("img[folder='Y']").bind("click", function(){
			...
			isOpen = (isOpen == "Y" || isOpen==undefined) ? "N" : "Y";
...
//			__root.find("tr[id^=" + _trid + "_]").css("display", "none");
			__root.find("tr[id^=" + _trid + "_]").each(function(){
				$(this).prop("open", _open);
				$(this).css("display", "none");
				$(this).find("img[folder='Y']").prop("src", folderCloseIcon);
			});
			$("#controlBtn").val("展开所有节点");
			$("#controlBtn").prop("name","openNode");
		}else{ //显示子节点
			__root.find("#"+_trid).find("img[folder='Y']").prop("src", folderOpenIcon);
			showSubs(_trid);
			$("#controlBtn").val("关闭所有节点");
			$("#controlBtn").prop("name","closeNode");
		}
	}
	//显示或隐藏子节点数据
	showHiddenNode2 = function(_trid, _open){
		__root.find("#"+_trid).prop("open", _open);
		if(_open == "N"){ //隐藏子节点
			__root.find("#"+_trid).find("img[folder='Y']").prop("src", folderCloseIcon);
//			__root.find("tr[id^=" + _trid + "_]").css("display", "none");
			__root.find("tr[id^=" + _trid + "_]").each(function(){
	...
	showSubs2 = function(_trid){
		var isOpen = __root.find("#" + _trid).prop("open");
		if(isOpen == "Y"){
			__root.find("tr[id^=" + _trid + "_]").each(function(){
				$(this).prop("open", isOpen);
				$(this).css("display", "");
				$(this).find("img[folder='Y']").prop("src", folderOpenIcon);
			});
		}
	}
