...
package cn.tz.www.admin.controller.handler;
...
import cn.tz.www.customer.entity.tools.BaseResponse;
...
public class ControllerExceptionHandler extends AbstractErrorController {
	...
	public ModelAndView serverError(HttpServletRequest req, HttpServletResponse rsp, Exception ex) throws IOException {
		LOGGER.error("请求地址:{},客户端IP:{},发生500错误:{}", req.getRequestURI(), CommonUtil.getIpAddress(req), ex.getMessage(),
				ex);
	...
	public ModelAndView notFound(HttpServletRequest request, HttpServletResponse response, Exception ex)
			throws IOException {
		LOGGER.debug("请求地址:{},客户端IP:{},发生404错误:{}", request.getRequestURI(), CommonUtils.getIpAddress(request),
				ex.getMessage());
		ObjectMapper mapper = new ObjectMapper();
		mapper.setSerializationInclusion(Include.NON_NULL);
	...
	public ResponseEntity<Map<String, Object>> error(HttpServletRequest request, HttpServletResponse response) {
		LOGGER.debug("请求地址:{},客户端IP:{},发生未知错误:{}", request.getRequestURI(), CommonUtils.getIpAddress(request),
